...
| `rolesync` | Applies the bot's Discord-role snapshots to a player's LuckPerms groups, on join and whenever someone's Discord roles change, and reports in-game rank changes back for Minecraft-to-Discord mappings. Does not need the whitelist module | every role | on |
...
**Give Discord roles for in-game ranks.** Role sync also runs the other way: a rank a player gets in game (bought through a store, or granted or removed with `/lp user ... parent`) can grant or remove a Discord role. This needs LuckPerms on the server. There is nothing to set on the plugin: which groups are watched is decided by the bot from the mappings you give a Minecraft-to-Discord direction in the dashboard, and the plugin reports only changes that touch one of those groups (with no such mappings it does not even listen). Changes are reported live, a few at once collapsed into one report, while the player is online and the server is connected to the bot. Anything missed (a change while the player was offline, a temporary rank that expired while they were away, or the bot being unreachable at the time) is reconciled at their next join, because the join-time request always carries the player's current groups. The plugin never removes a group because of this direction, so a purchased rank cannot be taken away by a sync.
